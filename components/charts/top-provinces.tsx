...
export function TopProvinces({ data }: TopProvincesProps) {
  ...
  const provinceData = useMemo(() => {
    ...
    // Group by province and calculate total investment
    const provinceMap = new Map<string, { pmdn: number; pma: number }>()
    ...
    data.forEach((item) => {
      ...
      if (!provinceMap.has(province)) {
        provinceMap.set(province, { pmdn: 0, pma: 0 })
      }
